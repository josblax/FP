...
ActuaríaSiniestralidad: Explica qué es un siniestro en el contexto de seguros y por qué es fundamental modelar su frecuencia y severidad para el cálculo de primas.Tasa de Interés: Diferencia conceptualmente el interés simple del interés compuesto; ¿por qué este último es la base de la valuación financiera a largo plazo?Tabla de Mortalidad: ¿Qué representa la función $q_x$ en una tabla de mortalidad y por qué es una estructura de datos inmutable para el actuario?Reserva Técnica: Investiga qué es una reserva técnica y por qué un actuario debe clasificar los fondos por ramos de seguro (Vida, Autos, Gastos Médicos).Auditoría de Pólizas: ¿Qué significa la "limpieza de datos" en una migración de sistemas legados a nuevos sistemas de gestión actuarial?Reincidencia: En el análisis de siniestros, ¿por qué es vital identificar clientes reincidentes (intersección de siniestros en diferentes años) para la prevención de fraudes?Esperanza Matemática: Explica cómo se calcula la esperanza matemática de un riesgo y cómo esta estructura permite a la aseguradora ser solvente.Modelo de Valor Presente: ¿Qué significa traer flujos de dinero futuros a valor presente y qué papel juega la tasa de descuento en este cálculo?Modelos Estocásticos: Define brevemente qué es un proceso estocástico y por qué los actuarios los usan para simular eventos que no tienen una certeza absoluta.Riesgo: ¿Cómo se relaciona la teoría de conjuntos con la clasificación de riesgos en una población asegurada? (Ej. intersectar clientes fumadores con clientes mayores a 60 años).📊 Ciencia de DatosFeature Engineering: ¿Qué es la ingeniería de características (feature engineering) y por qué es considerada la etapa más importante en la creación de un modelo predictivo?Loss Function: Define qué es una función de pérdida (loss function) y cuál es su objetivo durante el entrenamiento de un modelo de Machine Learning.One-Hot Encoding: ¿Por qué la mayoría de los algoritmos de aprendizaje automático requieren que las variables categóricas (como ciudades o colores) sean transformadas mediante One-Hot Encoding?NLP (Procesamiento de Lenguaje Natural): Explica qué es el análisis de frecuencia de palabras (como Bag of Words) y para qué sirve en el análisis de sentimiento.Dataset: ¿Qué es un conjunto de datos (dataset) y cuál es la diferencia entre los datos de entrenamiento (train set) y los de prueba (test set)?Data Leakage: Define qué es la "fuga de datos" (data leakage) y por qué invalida los resultados de un experimento de ciencia de datos.Outliers: ¿Qué es un valor atípico (outlier) en un conjunto de datos y qué impacto tiene sobre la media aritmética?Normalización: Explica por qué es necesario normalizar datos numéricos (como ingresos que van de 10k a 100k) antes de alimentar un modelo matemático.Matriz: ¿Por qué el álgebra lineal, representada mediante matrices o tensores, es el motor computacional detrás de la Inteligencia Artificial?Exactitud vs. Precisión: Diferencia entre los conceptos de exactitud (accuracy) y precisión (precision) al evaluar un modelo de clasificación.
